...
<div class="flex items-center justify-between">
    <div class="py-5">
        <h2 class="text-xl font-semibold">Tabel Diagnosa Pasien</h2>
    </div>
    <div class="flex gap-2">
        <!-- Tambah Owner & Pasien Baru -->
        <x-link color="blue" href="{{ route('masterdata.diagnosa.create') }}" class="font-bold w-[202px] py-5">Tambah Diagnosa</x-link>

        <!-- Tambah Owner /Pasien Lama -->
        <x-button data-modal-target="tambah-bulk" data-modal-toggle="tambah-bulk" type="button" color="" class="bg-[#EFF6FF] text-klinikBlue font-bold w-[202px] h-[56.8px] border border-bulkStroke">Tambah Bulk</x-button>
        ...
        <div class="relative w-full p-4 overflow-x-auto border-none sm:rounded-lg">
                ...
                <table class="min-w-full text-sm text-left text-gray-500 dark:text-gray-400" id="dataTable">
                    <thead class="text-gray-500 bg-gray-100 border-b border-gray-200">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 font-semibold text-gray-600 border-r border-gray-100"
                                style="font-size: 0.81rem">Kode Diagnosa</th>
                            <th scope="col" class="px-6 py-3 font-semibold text-gray-600 border-r border-gray-100"
                                style="font-size: 0.81rem">Nama Diagnosa</th>
                            <th scope="col" class="px-6 py-3 font-semibold text-gray-600 border-r border-gray-100"
                                style="font-size: 0.81rem">Jenis Hewan</th>
                    ...
                    <!-- <tr class="bg-white border-b">
                            ...
                            <td class="px-6 py-2 font-semibold text-center border-r border-gray-100">
                                ANJ-1</td>
                            <td class="px-6 py-2 font-semibold border-r border-gray-100 ">Diagnosa 1</td>
                            <td class="px-6 py-2 border-r border-gray-100 ">Anjing</td>
                            <td class="px-6 py-2 text-center border-r border-gray-100">
                                <div>
                                    <x-link color="" href="{{ route('masterdata.diagnosa.edit',1) }}" class="bg-[#EFA006]">
                        ...
                        @foreach ($diagnosa as $index => $item)
                            <tr class="bg-white border-b">
                                <td class="px-6 py-2 font-semibold border-r border-gray-100">
                                    {{ ($diagnosa->currentPage() - 1) * $diagnosa->perPage() + $index + 1 }}</td>
                                <td class="px-6 py-2 font-semibold text-center border-r border-gray-100">
                                    {{ $item->id_diagnosa }}</td>
                                <td class="px-6 py-2 font-semibold border-r border-gray-100 ">{{ $item->hasil_diagnosa }}</td>
                                <td class="px-6 py-2 border-r border-gray-100 ">{{ $item->hewan ? $item->hewan->jenis_hewan : 'Tidak ada data' }}</td>
                                <td class="px-6 py-2 text-center border-r border-gray-100">
                                    <div class="flex justify-center space-x-1">
                                        <form action="{{ route('masterdata.diagnosa.edit', $item->id_diagnosa) }}">
                                            @csrf
                                        ...
                                        <form action="{{ route('masterdata.diagnosa.destroy', $item->id_diagnosa) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <x-button onclick="return confirm('Yakin mau dihapus?')" type="submit" color="red"
                                                class="font-medium text-white transition-all duration-200 ease-in-out transform bg-alert rounded-lg shadow-lg focus:outline-none hover:bg-red-800 focus:ring-4 focus:ring-red-300 hover:scale-105 hover:shadow-xl me-2" aria-hidden="true">
                                                <svg width="17" height="18" viewBox="0 0 17 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M5.73926 0V1H0.739258V3H1.73926V16C1.73926 16.5304 1.94997 17.0391 2.32504 17.4142C2.70012 17.7893 3.20882 18 3.73926 18H13.7393C14.2697 18 14.7784 17.7893 15.1535 17.4142C15.5285 17.0391 15.7393 16.5304 15.7393 16V3H16.7393V1H11.7393V0H5.73926ZM5.73926 5H7.73926V14H5.73926V5ZM9.73926 5H11.7393V14H9.73926V5Z" fill="#F8F8F8"/>
                                                </svg>
                                            </x-button>
            ...
            <div class="flex items-center justify-between mt-4 text-sm text-gray-700">
                ...
                <div class="flex flex-row">
                    ...
                    <span class="m-2">
                        {{ ($diagnosa->currentPage() - 1) * $diagnosa->perPage() + 1 }} -
                        {{ min($diagnosa->currentPage() * $diagnosa->perPage(), $diagnosa->total()) }}
                        dari {{ $diagnosa->total() }}
                    </span>
                    ...
                    <div class="flex items-center">
                        ...
                        <button
                            ...
                            @if ($diagnosa->onFirstPage()) disabled @endif
                            onclick="window.location='{{ $diagnosa->previousPageUrl() }}'">
                            &lt;
                        </button>

                        <!-- Nomor Halaman -->
                        @foreach (range(1, $diagnosa->lastPage()) as $page)
                            @if ($page == $diagnosa->currentPage())
                            ...
                            @else
                                <button
                                    ...
                                    onclick="window.location='{{ $diagnosa->url($page) }}'">
                                    {{ $page }}
                                </button>
                            @endif
                        @endforeach
                        ...
                        <button
                            class="px-2 py-1 mx-1 text-gray-500 bg-white border border-gray-300 rounded hover:bg-gray-100 disabled:text-gray-300 disabled:cursor-not-allowed"
                            @if (!$diagnosa->hasMorePages()) disabled @endif
                            onclick="window.location='{{ $diagnosa->nextPageUrl() }}'">
                            &gt;
                        </button>
                    </div>
                </div>
