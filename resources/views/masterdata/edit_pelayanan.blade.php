...
@section('content')
    <div class="flex-row px-6 py-3 m-4 border-2 rounded-xl">
        <div class="flex"><b>Edit Jenis Pelayanan</b></div>
    </div>

    <div class="flex flex-col px-6 py-3 m-4 border-2 rounded-xl">
        <form action="{{ route('masterdata.pelayanan.update', $pelayanan->id_pelayanan) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="grid items-center w-full grid-cols-4 gap-4 mb-4">
            <label for="" class="text-sm font-medium leading-6 text-gray-700">Jenis Pelayanan</label>
            <input type="text" id="" name="jenis_pelayanan" value="{{ $pelayanan->jenis_pelayanan }}"
                class="w-full col-span-3 py-2 pl-3 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                required>
            </div>

            <div class="grid items-center w-full grid-cols-4 gap-4 mb-4">
                <label for="" class="text-sm font-medium leading-6 text-gray-700">Harga</label>
                <input type="number" id="" name="harga" value="{{ $pelayanan->harga }}"
                    class="w-full col-span-3 py-2 pl-3 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    required>
            </div>
