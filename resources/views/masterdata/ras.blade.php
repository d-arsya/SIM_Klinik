...
<div class="flex items-center justify-between">
    <div class="py-5">
        <h2 class="text-xl font-semibold">Tabel Ras Hewan</h2>
    </div>
    <div class="flex gap-2">
        <!-- Tambah Owner & Pasien Baru -->
        <x-link color="blue" href="{{ route('masterdata.ras.create') }}" class="font-bold w-[202px] py-5">Tambah Ras Hewan</x-link>
        ...
        <div class="relative w-full p-4 overflow-x-auto border-none sm:rounded-lg">
                ...
                <table class="min-w-full text-sm text-left text-gray-500 dark:text-gray-400" id="dataTable">
                    <thead class="text-gray-500 bg-gray-100 border-b border-gray-200">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 font-semibold text-gray-600 border-r border-gray-100"
                                style="font-size: 0.81rem">Kode Ras</th>
                            <th scope="col" class="px-6 py-3 font-semibold text-gray-600 border-r border-gray-100"
                                style="font-size: 0.81rem">Nama Ras</th>
                            <th scope="col" class="px-6 py-3 font-semibold text-gray-600 border-r border-gray-100"
                    ...
                    <tbody>
                        <!-- <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="px-6 py-2 font-semibold text-center border-r border-gray-100">
                                RAS-1</td>
                            <td class="px-6 py-2 font-semibold border-r border-gray-100 ">Ras Hewan A</td>
                        ...
                        @foreach ($ras as $index => $item)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td class="px-6 py-4 font-semibold border-r border-gray-100">
                                    {{ ($ras->currentPage() - 1) * $ras->perPage() + $index + 1 }}</td>
                                <td class="px-6 py-4 font-semibold text-center border-r border-gray-100">
                                    {{ $item->id_ras }}</td>
                                <td class="px-6 py-4 font-semibold border-r border-gray-100 ">{{ $item->nama_ras }}</td>
                                <td class="px-6 py-4 border-r border-gray-100 ">
                                    {{ $item->hewan ? $item->hewan->jenis_hewan : 'Tidak ada data' }}</td>
                                <td class="flex justify-center px-6 py-4 space-x-2 text-center border-r border-gray-100">
                                    <form action="{{ route('masterdata.ras.edit', $item->id_ras) }}">
                                        @csrf
                                    ...
                                    <form action="{{ route('masterdata.ras.destroy', $item->id_ras) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="flex items-center justify-between mt-4 text-sm text-gray-700">
                ...
                <div class="flex flex-row">
                    ...
                    <span class="m-2">
                        {{ ($ras->currentPage() - 1) * $ras->perPage() + 1 }} -
                        {{ min($ras->currentPage() * $ras->perPage(), $ras->total()) }}
                        dari {{ $ras->total() }}
                    </span>
                    ...
                    <div class="flex items-center">
                        ...
                        <button
                            ...
                            @if ($ras->onFirstPage()) disabled @endif
                            onclick="window.location='{{ $ras->previousPageUrl() }}'">
                            &lt;
                        </button>

                        <!-- Nomor Halaman -->
                        @foreach (range(1, $ras->lastPage()) as $page)
                            @if ($page == $ras->currentPage())
                            ...
                            @else
                                <button
                                    ...
                                    onclick="window.location='{{ $ras->url($page) }}'">
                                    {{ $page }}
                                </button>
                            @endif
                        @endforeach
                        ...
                        <button
                            class="px-2 py-1 mx-1 text-gray-500 bg-white border border-gray-300 rounded hover:bg-gray-100 disabled:text-gray-300 disabled:cursor-not-allowed"
                            @if (!$ras->hasMorePages()) disabled @endif
                            onclick="window.location='{{ $ras->nextPageUrl() }}'">
                            &gt;
                        </button>
                    </div>
                </div>
