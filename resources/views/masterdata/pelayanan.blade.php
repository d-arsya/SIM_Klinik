...
<div class="flex items-center justify-between">
    <div class="py-5">
        <h2 class="text-xl font-semibold">Tabel Jenis Pelayanan</h2>
    </div>
    <div class="flex gap-2">
        <!-- Tambah Owner & Pasien Baru -->
        <x-link color="blue" href="{{ route('masterdata.pelayanan.create') }}" class="font-bold w-[202px] py-5">Tambah Jenis Pelayanan</x-link>
        ...
        <div class="relative w-full p-4 overflow-x-auto border-none sm:rounded-lg">
                ...
                <table class="min-w-full text-sm text-left text-gray-500 dark:text-gray-400" id="dataTable">
                    <thead class="text-gray-500 bg-gray-100 border-b border-gray-200">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 font-semibold text-gray-600 border-r border-gray-100"
                                style="font-size: 0.81rem">Kode Pelayanan</th>
                            <th scope="col" class="px-6 py-3 font-semibold text-gray-600 border-r border-gray-100"
                                style="font-size: 0.81rem">Nama Pelayanan</th>
                            <th scope="col" class="px-6 py-3 font-semibold text-gray-600 border-r border-gray-100"
                                style="font-size: 0.81rem">Harga</th>
                    ...
                    <!-- <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="px-6 py-2 font-semibold text-center border-r border-gray-100">
                                SER-1</td>
                            <td class="px-6 py-2 font-semibold border-r border-gray-100 ">UGD</td>
                            <td class="px-6 py-2 border-r border-gray-100 ">Rp 18.000</td>
                        ...
                        @foreach ($pelayanan as $index => $item)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td class="px-6 py-4 font-semibold border-r border-gray-100">
                                    {{ ($pelayanan->currentPage() - 1) * $pelayanan->perPage() + $index + 1 }}</td>
                                <td class="px-6 py-4 font-semibold text-center border-r border-gray-100">
                                    {{ $item->id_pelayanan }}</td>
                                <td class="px-6 py-4 font-semibold border-r border-gray-100 ">{{ $item->jenis_pelayanan }}</td>
                                <td class="px-6 py-4 font-semibold border-r border-gray-100 ">
                                    Rp {{ number_format($item->harga, 2, ',', '.') }}
                                </td>
                                <td class="flex justify-center px-6 py-4 space-x-2 text-center border-r border-gray-100">
                                    <form action="{{ route('masterdata.pelayanan.edit', $item->id_pelayanan) }}">
                                        @csrf
                                    ...
                                    <form action="{{ route('masterdata.pelayanan.destroy', $item->id_pelayanan) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="flex items-center justify-between mt-4 text-sm text-gray-700">
                ...
                <div class="flex flex-row">
                    ...
                    <span class="m-2">
                        {{ ($pelayanan->currentPage() - 1) * $pelayanan->perPage() + 1 }} -
                        {{ min($pelayanan->currentPage() * $pelayanan->perPage(), $pelayanan->total()) }}
                        dari {{ $pelayanan->total() }}
                    </span>
                    ...
                    <div class="flex items-center">
                        ...
                        <button
                            ...
                            @if ($pelayanan->onFirstPage()) disabled @endif
                            onclick="window.location='{{ $pelayanan->previousPageUrl() }}'">
                            &lt;
                        </button>

                        <!-- Nomor Halaman -->
                        @foreach (range(1, $pelayanan->lastPage()) as $page)
                            ...
                            @else
                                <button
                                    ...
                                    onclick="window.location='{{ $pelayanan->url($page) }}'">
                                    {{ $page }}
                                </button>
                            @endif
                        @endforeach
                        ...
                        <button
                            class="px-2 py-1 mx-1 text-gray-500 bg-white border border-gray-300 rounded hover:bg-gray-100 disabled:text-gray-300 disabled:cursor-not-allowed"
                            @if (!$pelayanan->hasMorePages()) disabled @endif
                            onclick="window.location='{{ $pelayanan->nextPageUrl() }}'">
                            &gt;
                        </button>
                    </div>
                </div>
